...
GO
CREATE FUNCTION [dbo].[fn_GetAvgForestryGrowthPerHectare] (@ForestryID UNIQUEIDENTIFIER)
RETURNS FLOAT
AS
BEGIN
	...
	;WITH [GrowthAreas] AS 
	(
		SELECT SUM([TG].[Volume]) / [dbo].[fn_GetAvgAge](@ForestryID, [TG].[TreeSpeciesID]) AS [GrowthPart],
